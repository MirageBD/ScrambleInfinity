...
diskioreq:  .tag diskio::IMPORT
            ...
            ; and also redefine the symbols to point to dynlink module space;
...
:
    ...
    .if !LOAD_ONCE
            ...
            bcc :+
            ldx #diskio::drivetypes::DRIVE_1581 + 1
:           ldy paramoffst - diskio::drivetypes::DRIVE_1541,x
            lda xshebaunpt,y
...
:           ora #'0'
            ...
            ldx DRIVETYPE
            bpl :+
            ldx #diskio::drivetypes::DRIVE_1581 + 1; generic drive
:           lda drivtypesl - diskio::drivetypes::DRIVE_1541,x
            ldy drivtypesh - diskio::drivetypes::DRIVE_1541,x
            tax
            jsr plottext
...
:           ; imagine some data processing here
    ...
    .elseif LOAD_UNCOMPD_API
            ; --- load uncompressed ---
        .if LOAD_TO_API
            LOADUNCOMPD_LOADTO #.lobyte(BITMAP + $20), #.hibyte(BITMAP + $20)
        .else
            LOADUNCOMPD
        .endif
            jcs error
...
.if OPEN_FILE_POLL_BLOCK_API
    ...
    .macro LOADPOLLED xsource, ysource
        ...
        .if LOAD_TO_API
            OPENFILE_LOADTO xsource, ysource, #.lobyte(BITMAP + $40), #.hibyte(BITMAP + $40)
        .else
...
.if LOAD_UNCOMPD_API
    ...
    .macro LOADUNCOMPRESSED xsource, ysource
        ...
        .if LOAD_TO_API
            LOADUNCOMPD_LOADTO xsource, ysource, #.lobyte(BITMAP + $20), #.hibyte(BITMAP + $20)
        .else
            LOADUNCOMPD xsource, ysource
        .endif
            jcs error
...
:           POLLBLOCK
            ...
            jne error
        .else
            .error "To test memory decompression, please set LOAD_UNCOMPD_API or OPEN_FILE_POLL_BLOCK_API to non-0"
        .endif

        .if MEM_DECOMP_TO_API
            MEMDECOMP_TO loadaddrl, loadaddrh, #.lobyte(BITMAP - $10), #.hibyte(BITMAP - $10)
        .else
            MEMDECOMP loadaddrl, loadaddrh
        .endif
...
printstat:  lda #~TIMER_STOP
            ...
            jeq ntsckbs
            LONGTOFAC numccpal; fac#1 = cyclesperframe
            jmp :+
ntsckbs:    LONGTOFAC numccntsc; fac#1 = cyclesperframe

:           lda #.lobyte(floatbuff2)
            ldy #.hibyte(floatbuff2)
            jsr FDIVT; fac#1 /= numcycles
            lda #.lobyte(floatbuff)
            ldy #.hibyte(floatbuff)
            jsr FDIVT; fac#1 /= numbytes

            jsr DIV10; /= 10
            jsr DIV10; /= 10
            jsr DIV10; /= 10

            ; fac#1 = cyclesperframe / numcycles / numbytes / 1000
...
drivtypesl: .byte .lobyte(str1541), .lobyte(str1541c), .lobyte(str1541ii)
            .byte .lobyte(str1570), .lobyte(str1571), .lobyte(str1581)
            .byte .lobyte(strgeneric)

drivtypesh: .byte .hibyte(str1541), .hibyte(str1541c), .hibyte(str1541ii)
...
.if (LOAD_ONCE = 0) && (FILESYSTEM = FILESYSTEMS::DIRECTORY_NAME)
...
paramoffst: .byte diskio::drivetypes::DRIVES_1541    ; 1541
            .byte diskio::drivetypes::DRIVES_1541    ; 1541-C
            .byte diskio::drivetypes::DRIVES_1541    ; 1541-II
            .byte diskio::drivetypes::DRIVES_157X    ; 1570
            .byte diskio::drivetypes::DRIVES_157X    ; 1571
            .byte diskio::drivetypes::DRIVES_1581_CMD; 1581
            .byte diskio::drivetypes::DRIVES_1581_CMD; generic drive
...
errormsgsl: .byte .lobyte(invaliderr)
...
UNEXPCNMI = * - errormsgsl
            ...
            .byte .lobyte(emsgfnotop), .lobyte(emsgparerr), .lobyte(emsgdevnp), .lobyte(emsgdevinc), .lobyte(emsgtoomd), .lobyte(emsggenker)
            .byte .lobyte(eunspecerr)

errormsgsh: .byte .hibyte(invaliderr)
            ...
            .byte .hibyte(emsgfnotop), .hibyte(emsgparerr), .hibyte(emsgdevnp), .hibyte(emsgdevinc), .hibyte(emsgtoomd), .hibyte(emsggenker)
