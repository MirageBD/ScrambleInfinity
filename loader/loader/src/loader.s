...
.endmacro
            ...
            ;      c - load to caller-specified address if set
            ; out: c - set on error
            ;      a - status
.ifdef openfile
openfile2:
...
getloadadr: jsr get1byte; block size
            pha
...
.ifdef DYNLINKLDR
            .assert .hibyte(* + 1) = .hibyte(getblkloop), warning, "***** Performance warning: page boundary crossing (getblkloop). Please relocate the LOADER segment a few bytes up or down. *****"
.endif
...
.if MEM_DECOMP_API
            ...
            ; in:  x/a - lo/hi of compressed file in memory
...
:
    ...
    .if LOAD_VIA_KERNAL_FALLBACK
        ...
        .if CHAINED_COMPD_FILES
            jmp :++
:           jsr kernalgetc; skip load address
            jsr kernalgetc
:           jsr decompress
            jmp :--
        .else
...
loaderrts:  cmp #diskio::status::OK + 1; sets carry flag according to the error state
stackpntr:  ldx #$00
            txs
    .if DECOMPRESSOR = DECOMPRESSORS::PUCRUNCH
            ldx lo+$01
...
getnewblk2: pha
            jsr getblock
            bcs lastblock; branch if block number is DEVICE_NOT_PRESENT ($00), LOAD_FINISHED ($fe), LOAD_ERROR ($ff, file not found or illegal t or s)
    .if CHAINED_COMPD_FILES
            ldx loadycomp+$01
            cpx dependlo+$01
            ldy storbyte+$02
