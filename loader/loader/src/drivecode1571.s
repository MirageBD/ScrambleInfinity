...
drvcodebeg: .byte .hibyte(drivebusy-*+$0100-$01); init transfer count hi-byte
dcodinit:   jsr drivebusy
            jsr two_mhz
...
.if !LOAD_ONCE ; not with LOAD_ONCE because then, there is no danger of getting stuck
            ...
            ENABLE_WATCHDOG; enable watchdog, the c64 might be reset while sending over
...
:           bit VIA1_PRB
            ...
            dec SECTORCOUNT
            bne drivebusy; pull DATA_OUT high when changing tracks
...
instalwait: bcc :-
            ...
            txs
            ldx #.lobyte(drvcodebeg-$01)
dgetrout:   inx
            bne :+
            inc dgetputhi
:           jsr dgetbyte
dgetputhi = *+$02
            sta a:.hibyte(drvcodebeg-$01) << 8,x
            cpx #.lobyte(drivebusy-$01)
            bne dgetrout
            dec drvcodebeg
            bne dgetrout
