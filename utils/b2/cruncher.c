...
bool crunch(File *aSource, File *aTarget, uint address, bool isExecutable, bool isRelocated)
{
	...
	if(isExecutable)
	{
		...
		decrCode[decrunchinitlength + asm_transferfrom + 1]	= transfAddress &  0xff;
		decrCode[decrunchinitlength + asm_transferfrom + 2]	= transfAddress >> 8;
		decrCode[decrunchinitlength + asm_depackfrom   + 1]	= startAddress  &  0xff;
		decrCode[decrunchinitlength + asm_depackfrom   + 2]	= startAddress  >> 8;
		decrCode[decrunchinitlength + asm_depackto     + 1]	= aSource->data[0];
		decrCode[decrunchinitlength + asm_depackto     + 2]	= aSource->data[1];
		decrCode[decrunchinitlength + asm_jumpto       + 1]	= address	&  0xff;
		decrCode[decrunchinitlength + asm_jumpto       + 2]	= address	>> 8;
